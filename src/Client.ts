...
export class FunctionsPlugin extends PluginClient {
  ...
  async setCallBacks() {
    ...
    this.on(
      "solidity",
      "compilationFinished",
      async (target, source, version, data) => {
        ...
        if (data) {
          if (data.contracts) {
            ...
            console.log(abi);
            // abi
            const functionSignatureString = getSignatureString(
              client.setHelloAbi
            );
    ...
    this.on(
      "udapp",
      "newTransaction",
      async (tx: RemixTxEvent, receipt: any) => {
        ...
        if (tx.from === signerAccount) {
          ...
          console.log("aem compare bytecode");
          console.log(input);
          console.log(client.byteCode);
  ...
  async transact(input: any) {
    ...
    const signerAccount = accounts[0];
    const dataTrans = getTransaction(this.setHelloAbi, [
      "hello" + 100 * Math.random(),
    ]);
